...
def classify_orbit_full(E, Lz, a, M=1.0):
    ...
    int : Classification code
        ...
        5: Deep ergosphere (deep-ergosphere diagnostic region)
    ...
    try:
        ...
        if r_peri >= r_erg:
            ...
            return 5, r_peri  # Deep ergosphere (diagnostic region)
    except:
...
def generate_figure_1(M=1.0, n_E=120, n_Lz=120, save=True):
    ...
    Panel (b): High spin a/M = 0.95 - shows deep-ergosphere region expansion
    
    This demonstrates how increased spin expands the deep-ergosphere
    diagnostic region at the expense of shallow/outside regions.
    """
    ...
    colors_list = [
        ...
        COLORS['green'],     # 5: Deep ergosphere (diagnostic region)
    ]
...
def generate_figure_2(results_dir=None, save=True):
    ...
    (b) Extraction-with-escape rate by spin with confidence intervals
    """
    print("Generating Figure 2: Ensemble Statistics (Ultra-Scale Sweep, ~800k trajectories)...")
    
    # Load data from fig2_ensemble_data.json (generated by generate_all_data.py)
    fig2_path = RESULTS_DIR / "fig2_ensemble_data.json"
    if fig2_path.exists():
        with open(fig2_path) as f:
            ensemble_data = json.load(f)
        broad_data = ensemble_data.get('broad', {})
        focused_data = ensemble_data.get('focused', {})
    else:
        # Fallback: try legacy data sources
        ...
        if sweep_data is not None:
            ...
            broad_data = ultra_data.get('broad', {})
            focused_data = ultra_data.get('focused', {})
        else:
            raise FileNotFoundError(
                f"No data for Figure 2. Run: python experiments/generate_all_data.py --fig2\n"
                f"Expected file: {fig2_path}"
            )
    ...
    ax.set_ylabel('Extraction-with-escape rate (%)', fontsize=10)
    ax.set_xticks(x)
...
def run_single_thrust_simulation():
    ...
    # Use parameters matching Table IV (single-impulse row)
    E0 = 1.20
    Lz0 = 3.0
    r0 = 15.0  # Match paper's r_0 = 15M
    delta_m_fraction = 0.20  # Match Table IV single-impulse δm (actual δm ≈ 0.22)
    ...
    def apply_impulse_for_single(state, dm_frac, v_exhaust):
        ...
        for alpha in np.linspace(-np.pi/2, np.pi/2, 37):
            ...
            for sign_phi in [+1.0, -1.0]:
                ...
                # Check unbound-energy filter (E > m_after)
    ...
    # Scan trigger radii to find optimal — dense scan near horizon for deep E_ex
    trigger_radii = np.concatenate([
        np.linspace(r_plus + 0.02, r_plus + 0.3, 40),   # Dense near horizon
        np.linspace(r_plus + 0.3, r_erg_eq - 0.1, 20),  # Sparser further out
    ])
    best_eta = -float('inf')
    best_result = None
...
def generate_figure_3(save=True):
    ...
    Reports η_cum (per rocket mass lost) matching Table IV in the paper.
    """
    ...
    # For continuous case, get E_ex at thrust locations (must come from real simulation)
    if len(cont_data['E_ex_history']) > 0 and 'r_thrust' in cont_data and len(cont_data['r_thrust']) > 0:
        ...
        raise RuntimeError(
            "Continuous-thrust simulation produced no E_ex data. "
            "Cannot generate Figure 3 panel (c) without real physics data."
        )
    ...
    # Use explicit LaTeX minus sign for clarity in PDF rendering
    Eex_val = Eex_single[0]
    if Eex_val < 0:
        Eex_label = f'Single ($E_{{{{ex}}}}={{-}}{abs(Eex_val):.3f}$)'
    else:
        Eex_label = f'Single ($E_{{{{ex}}}}={Eex_val:.3f}$)'
    legend_elements = [
    ...
    # Panel (d): Efficiency comparison - use η_cum (per rocket mass lost)
    # Matches Table IV and Eq. (eta_cum) in the paper
    ax = axes[1, 1]

    # Get actual efficiency from simulations — no fallback defaults
    if 'eta_cum' not in single_data:
        raise RuntimeError("Single-thrust simulation missing efficiency data")
    single_eta_cum = single_data['eta_cum'] * 100
    single_escaped = single_data['escaped']

    cont_eta_cum = cont_data.get('eta_cum', 0.0) * 100

    # Show η_cum (propellant-normalized) matching Table IV
    strategies = ['Single\nImpulse', 'Continuous\nThrust']
    efficiencies = [single_eta_cum, cont_eta_cum]
...
def generate_figure_5(save=True):
    ...
    (a) Extraction-with-escape rate vs v_e for different delta_m values with 95% CI
    ...
    for v in v_e_values:
        for dm in delta_m_values:
            ...
            if key in data:
                ...
                n_success = entry.get('n_penrose', 0)
                # Conditional mean is undefined when no successes exist;
                # use NaN so those bins appear as gaps, not zeros.
                if n_success > 0:
                    efficiency_data[dm].append(entry.get('eta_mean', 0) * 100)
                else:
                    efficiency_data[dm].append(np.nan)
            else:
    ...
    for dm in delta_m_values:
        ...
        ax.errorbar(v_e_values, success_data[dm], 
                    ...
                    label=f'$\\delta m_{{\\rm label}} = {dm:.1f}$')
    
    # Find onset velocity (where success jumps from ~0 to significant)
    ...
    # Add annotation for onset with velocity value
    ax.annotate(f'Sharp onset\n$v_e \\approx {v_transition:.2f}c$',
...
def generate_figure_6(save=True):
    ...
    # Sort by velocity (skip metadata key)
    v_e_set = sorted(set(entry['v_e'] for key, entry in data.items() if key != 'metadata'))
    ...
    # Create single-panel figure (trimmed per reviewer recommendation)
    fig, ax = plt.subplots(1, 1, figsize=(PRD_DOUBLE_COL * 0.55, 3.5))
    ...
    # Efficiency vs Lorentz factor (log scale)
    for dm in delta_m_values:
        color = dm_colors[dm]
    ...
    ax.set_ylabel(r'Cumulative efficiency $\eta_{\rm cum}$ (%)', fontsize=10)
    ax.set_title(r'Efficiency saturation ($a/M=0.95$)', fontsize=10)
    ax.set_xlim(2, 300)
    ax.set_ylim(-0.5, 12)
    ax.grid(True, alpha=0.3, which='both')
    ax.legend(fontsize=8, loc='lower right', ncol=2)
    axes = ax  # for return compatibility
...
def generate_all_figures():
    ...
    # Figure 5: Thrust parameter sensitivity (velocity onset threshold)
    generate_figure_5()
    print()
